...
if [ "$#" -ne 2 ]; then
    echo "Usage: ./4_bqload.sh  [BUCKET] [YEAR]"
    exit
fi
...
# Definir schema de BigQuery
# Sólo se define STRING para evitar errores de carga
...
# Crear dataset de BQ en caso de no existir
PROJECT=$(gcloud config get-value project)
bq --project_id $PROJECT show dsongcp || bq mk --sync dsongcp

# Ciclo de cargas a BigQuery de todos los meses para YEAR dado
for MONTH in `seq -w 1 12`;
do
    # Extraer archivo csv de bucket en Google Cloud
    CSVFILE=gs://${BUCKET}/flights/raw/${YEAR}${MONTH}.csv

    # Convertir a esquema de BigQuery y cargar
